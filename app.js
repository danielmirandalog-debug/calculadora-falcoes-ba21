...
function simularFaturamento() {
    ...
    let ecoTaxas = f * 0.022; 
    if(document.getElementById("check_pix_taxa").checked) {
        ecoTaxas += (f * (parseFloat(share_pix.value)/100)) * 0.01;
    }
    ...
    document.getElementById("resultadoFaturamento").innerHTML = `
        <div class="resumo-financeiro">
            ...
            <b>Saldo Cofrinho 1 Ano:</b> R$ ${calc(12).toFixed(2)}<br>
            <b>Saldo Cofrinho 5 Anos:</b> R$ ${calc(60).toFixed(2)}
        </div>`;
    
    if (window.g) window.g.destroy();
    window.g = new Chart(document.getElementById("graficoEconomia"), {
        type: 'bar',
        data: { labels: ["Economia 1a", "Economia 5a", "Cofre 5a"], datasets: [{ label: 'R$', data: [ecoMes*12, ecoMes*60, calc(60)], backgroundColor: ['#FFE600','#FFD400','#3483FA'] }] }
    });
}
...
async function processarOCR(event, pref) {
    ...
    if(!file) return;
    alert("Iniciando leitura da imagem...");
